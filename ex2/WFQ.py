...
parser.add_argument(
    ...
    "--bandwith",
    type=str,
    help="Bandwith for each flow (as a percentage)",
    required=True,
)
parser.add_argument(
    "-f", "--file", type=str, help="Path to the config file", required=True
)
    ...
    for pck in file_data:
        ...
        if queue == []:
            ...
            packet = Packet(
                ...
                pck[0],
                pck[1],
                calculate_f_initial_packet(pck[0], pck[1], bandwith[int(pck[2]) - 1]),
            )
            time = float(pck[0])
            queue.append(packet)
        else:
            ...
            if queue[0].arrival_time == pck[0]:
                ...
                packet = Packet(
                    ...
                    pck[0],
                    pck[1],
                    calculate_f_initial_packet(
                        pck[0], pck[1], bandwith[int(pck[2]) - 1]
                    ),
                )
                queue.append(packet)
            else:
                ...
                if float(pck[0]) <= time:
                    ...
                    packet = Packet(
                        ...
                        pck[0],
                        pck[1],
                        calculate_f_next_packet(
                            pck[0], pck[1], current_packet.f, bandwith[int(pck[2]) - 1]
                        ),
                    )
                    queue.append(packet)
                else:
